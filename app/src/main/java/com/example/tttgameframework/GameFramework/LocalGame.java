...
public abstract class LocalGame implements Game, Tickable {
    ...
    // the com.example.tttgameframework.tickettoride.players in the game, in order of  player number
    protected GamePlayer[] players;

    // whether the game's thread is running
    private boolean running = false;

    // the com.example.tttgameframework.tickettoride.players' names, paralleling the 'com.example.tttgameframework.tickettoride.players' array
    protected String[] playerNames;
    private int playerNameCount = 0; // number of com.example.tttgameframework.tickettoride.players who have told us their name

    // the com.example.tttgameframework.tickettoride.players are ready to start
    private boolean[] playersReady;
    private int playerReadyCount = 0; // number of com.example.tttgameframework.tickettoride.players who are ready to start the game

    // the com.example.tttgameframework.tickettoride.players which have acknowledged that the game is over
     ...
     *
     * @param players
     * 			the list of com.example.tttgameframework.tickettoride.players who are playing in the game
     */
    public void start(GamePlayer[] players) {
        ...
        // create an array for the com.example.tttgameframework.tickettoride.players' names; these names will be
        // filled during the initial message-protocol between the game
        // and com.example.tttgameframework.tickettoride.players
        ...
        // waiting for the com.example.tttgameframework.tickettoride.players to tell us their names
    ...
    /**
     * Notify all com.example.tttgameframework.tickettoride.players that the game's state has changed. Typically this simply
    ...
    private void receiveMessage(Message msg) {
        if (msg.obj instanceof GameAction) { // ignore if not GameAction
            ...
            // com.example.tttgameframework.tickettoride.players to tell us their names. In this case, we expect
            ...
            if (action instanceof MyNameIsAction &&
                ...
                // If all com.example.tttgameframework.tickettoride.players have told us their name, then move onto the next
                ...
                if (playerNameCount >= playerNames.length) {
                    ...
                    playersReady = new boolean[players.length]; // array to keep track of com.example.tttgameframework.tickettoride.players responding
                    for (GamePlayer p : players) {
                        p.sendInfo(
                ...
                // CASE 2: we have told all com.example.tttgameframework.tickettoride.players that the game is about to start;
                ...
                // if all com.example.tttgameframework.tickettoride.players are ready, set the game stage to "during game", and
    ...
    private final void checkAndHandleAction(GameAction action) {
        ...
        // changed. Send all com.example.tttgameframework.tickettoride.players the updated state.
        sendAllUpdatedState();
    ...
    private final void finishUpGame(String msg) {
        ...
        // send all com.example.tttgameframework.tickettoride.players a "game over" message
        for (GamePlayer p : players) {
